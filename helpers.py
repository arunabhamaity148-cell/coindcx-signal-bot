            ...
            try:
                for sym in CFG["pairs"]:
                    try:
                        ...
                        for t in trades[-100:]:
                            await r.lpush(f"tr:{sym}", json.dumps({"p": t['price'], "q": t['amount'], "m": t['side'] == 'sell'}))
...
async def mtf_trend(sym):
    ...
    for tf in ["1m", "5m"]:
        ...
        trends.append(1 if df["c"].ewm(span=20).mean().iloc[-1] > df["c"].ewm(span=50).mean().iloc[-1] else -1)
...
async def extract_features(sym):
    try:
        ...
        if df_1m is None or len(df_1m) < 50: "large_sells", "spread", "depth", "mtf_trend", "reg_trend", "reg_chop", "funding", "poc_dist"]
...
async def calculate_advanced_score(sym, strategy):
    try:
        ...
        scores = {
            ...
            "mtf": 9.0 if (features["mtf_trend"] * 2 - 1) > 0.5 else 1.0 if (features["mtf_trend"] * 2 - 1) < -0.5 else 5.0,
        ...
        if not (cfg["min_score"] <= total <= cfg["max_score"]):
            side = "none"
        return {"score": round(total, 2), "side": side, "components": scores, "last": last, "features": features, "strategy": strategy}
    except Exception as e:
