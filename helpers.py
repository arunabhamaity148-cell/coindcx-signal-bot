...
# ----------------- ENV -----------------
SCORE_MIN            = int(os.getenv("SCORE_MIN", "90"))   # এখন টেস্টের জন্য 30
...
def high_impact_now(minutes: int = 15) -> bool:
    try:
        ...
        for _, row in cal.iterrows():
            if row["impact"] == "High" and abs(
                (row["time"] - now).total_seconds()
            ) < minutes * 60:
                return True
...
def ema21_pullback(df: pd.DataFrame) -> bool:
    if len(df) < 22:
        return False
    return abs(df["close"].iloc[-1] - ema(df["close"], 21).iloc[-1]) / df[
        "close"
    ].iloc[-1] < 0.002
...
def range_break_retest(df: pd.DataFrame) -> bool:
    ...
    return (close_last > high_line and low_last <= high_line) or (
        close_last < low_line and low_last >= low_line
    )
...
def log_signal(sig: Dict[str, Any]):
    os.makedirs("logs", exist_ok=True)
...
def format_signal(sig: Dict[str, Any], no: int) -> str:
    symbol   = sig["symbol"]
    mode_raw = str(sig["mode"]).lower()     # quick / mid / trend
    mode     = mode_raw.upper()             # display
    side     = sig["side"]
    score    = sig["score"]
    entry    = sig["entry"]
    tp       = sig["tp"]
    ...
    base_dt  = datetime.strptime(time_utc, "%Y-%m-%d %H:%M:%S")
    ist_time = (base_dt + timedelta(hours=5, minutes=30)).strftime("%I:%M %p")

    hold_map = {"quick": 15, "mid": 45, "trend": 120}
    hold_min = hold_map.get(mode_raw, 60)
    hold_till = (base_dt + timedelta(minutes=hold_min)).strftime("%I:%M %p")
...
COIN_LIST: List[str] = [
    ...
    "MATICUSDT", "DOTUSDT", "LTCUSDT", "BCHUSDT", "AVAXUSDT", "UNIUSDT", "LINKUSDT",
    "ATOMUSDT", "ETCUSDT", "FILUSDT", "ICPUSDT", "NEARUSDT", "APTUSDT", "SANDUSDT",
    "AXSUSDT", "THETAUSDT", "FTMUSDT", "RUNEUSDT", "ALGOUSDT", "EGLDUSDT", "IMXUSDT",
    "INJUSDT", "OPUSDT", "ARBUSDT", "SUIUSDT", "TIAUSDT", "PEPEUSDT", "TRBUSDT",
