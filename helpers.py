...
# - includes ATR, indicators, 58 logic descriptions
# - robust OpenAI calling with safe JSON parsing
# - deterministic fallback heuristic when LLM response not parseable
# - relaxed filters as requested
import os
import time
import json
import html
...
def _call_openai_sync(prompt: str, model: Optional[str] = None, max_tokens: int = 300) -> str:
    try:
        ...
        # fallback: return as json string if available
        try:
            return json.dumps(resp.to_dict())
        except Exception:
            return str(resp)
    except Exception as e:
        logging.debug("OpenAI call failed: %s", e)
        return ""

# ---------- deterministic heuristic fallback ----------
def heuristic_score_from_metrics(base: int, metrics: dict) -> dict:
    # simple deterministic scoring to avoid "always 0"
    # base [0..20] from HTF alignment. Add RSI contribution and volume boost.
    rsi = metrics.get("rsi_1m", 50)
    vol = metrics.get("vol_1m", 0)
    spread = metrics.get("spread_pct", 999)
    score = base
    # RSI adds up to +20 (for RSI 70 -> +10, RSI 80 -> +20)
    score += max(0, min(20, int((rsi - 50) * 0.5)))
    # volume adds small boost if large (normalized)
    if vol and vol > 0:
        score += 5 if vol > 10000 else (3 if vol > 2000 else 0)
    ...
    if score > 85:
        ...
        mode = "quick"
    reason = "heuristic signal" if score > 0 else "no signal"
    return {"score": score, "mode": mode, "reason": reason}

# ---------- relaxed AI scorer with robust parsing + fallback ----------
async def ai_score_symbol_with_memory_and_tools(symbol: str, price: float, metrics: dict, prefs: dict, ttl: int = 120) -> Optional[dict]:
    ...
    try:
        ...
        if parsed and isinstance(parsed, dict) and {"score", "mode", "reason"} <= set(parsed.keys()):
            try:
                # normalize types and validate optionally
                out = {
                ...
                logging.debug("Parsed but normalization failed: %s", e)
    except Exception as e:
        logging.debug("AI thread call exception: %s", e)

    # fallback deterministic heuristic (important: prevents constant 0)
    heur = heuristic_score_from_metrics(base, metrics)
    CACHE.set(key, heur, ttl_seconds=ttl)
    append_memory(symbol, heur)
    logging.info("Fallback heuristic used for %s -> %s", symbol, heur)
    return heur
...
async def ensemble_score(symbol: str, price: float, metrics: dict, prefs: dict, n: int = 3) -> dict:
    ...
    return {"score": final_score, "mode": final_mode, "reason": scores[0].get("reason", "")}
