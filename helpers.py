"""
helpers.py — ML models, feature extraction, orderflow, exchange wrapper
Production-ready with 70%+ accuracy targeting
"""
...
class WS:
    ...
    async def run(self):
        ...
        while self.running and retry < 10:
            try:
                log.info(f"🔌 Connecting to Binance WebSocket (attempt {retry+1})...")
                        ...
                        async for msg in ws:
                            if msg.type == aiohttp.WSMsgType.TEXT:
                                try:
                                    ...
                                    log.warning(f"Message handler error: {e}")
                ...
                retry += 1
                wait = min(retry * 2, 30)
                log.error(f"WebSocket error: {e}. Retrying in {wait}s...")
    ...
    async def _handle(self, data):
        ...
        try:
            ...
            if "@ticker" in stream:
                ...
                await r.lpush(f"tr:{s}", trade)
                await r.ltrim(f"tr:{s}", 0, 499)
                ...
                if k.get("x"):  # closed candle
                    ...
                    await r.lpush(f"kline_{tf}:{s}", kline)
                    await r.ltrim(f"kline_{tf}:{s}", 0, 199)
...
async def mtf_trend(sym):
    """Multi-timeframe trend (1m, 5m, 15m)"""
    try:
        trends = []
        for tf in ["1m", "5m", "15m"]:
            df = await get_ohlcv(sym, tf, limit=50)
...
async def orderflow_analysis(sym):
    """Orderflow: delta, imbalance, large trades"""
    try:
        r = await redis()
        ...
        # Delta
        delta = sum(
            float(t["q"]) if t["m"] == "false" else -float(t["q"])
            for t in trades
        )
        ...
        large_buys = sum(
            1 for t in trades[:20]
            if t["m"] == "false" and float(t["q"]) > avg_vol + 2*std_vol
        )
        large_sells = sum(
            1 for t in trades[:20]
            if t["m"] == "true" and float(t["q"]) > avg_vol + 2*std_vol
        )
...
async def regime(sym):
    """Regime: TREND/VOLATILE/CHOP"""
    try:
        df = await get_ohlcv(sym, "15m", limit=50)
...
async def ai_review_ensemble(sym, side, score):
    """Ensemble ML + strict filters for 70%+ accuracy"""
    try:
        # Get features
        ...
        if not models:
            # Fallback: rule-based with lower confidence
        ...
        for model in models:
            try:
                ...
                predictions.append(proba[1] if len(proba) > 1 else 0.5)
            except:
...
async def calculate_advanced_score(sym):
    ...
    try:
        ...
        # Orderflow
        delta_mom = features["delta_momentum"]
        imbalance = (features["imbalance"] - 0.5) * 2
...
async def calc_smart_tp_sl(sym, side, entry):
    """Dynamic TP/SL based on ATR + regime"""
    try:
        atr_1m = await get_real_atr(sym, "1m", 14)
        atr_5m = await get_real_atr(sym, "5m", 14)
...
async def check_risk_limits(equity, open_positions):
    ...
    try:
        ...
        total_exposure = sum(abs(p['size']) * p['entry'] for p in open_positions.values())
        if total_exposure > equity * 0.6:
            return False, "max-exposure"
        
        r = await redis()
        daily_pnl = float(await r.get("daily_pnl") or 0)
        if daily_pnl < -equity * 0.05:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    url,
                    json={"chat_id": CFG["tg_chat"], "text": txt, "parse_mode": "HTML"},
...
class Exchange:
    ...
    async def set_leverage(self, sym, leverage):
        try:
            await self.ex.fapiPrivate_post_leverage({
                'symbol': sym.replace('/', ''),
                'leverage': leverage
            })
    ...
    async def set_sl_tp(self, sym, side, sl, tp):
        try:
            ...
            await self.ex.create_order(
                sym, "STOP_MARKET", sl_side, None,
                params={"stopPrice": sl, "closePosition": True}
            )
            
            tp_side = "SELL" if side == "long" else "BUY"
            await self.ex.create_order(
                sym, "TAKE_PROFIT_MARKET", tp_side, None,
