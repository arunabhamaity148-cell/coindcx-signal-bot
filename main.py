...
# main.py – ULTIMATE FINAL v5.0 (All Features Integrated)
# ================================================================
...
async def send_telegram(message: str, chart_url: str = None, retry: int = 3):
    """
    Send Telegram notification with optional chart image
    ...
    # Send message with chart as photo if URL provided
    ...
    for attempt in range(retry):
        try:
            ...
            log.warning(f"Telegram timeout (attempt {attempt + 1}/{retry})")
...
async def ws_worker(pairs_chunk: list, worker_id: int):
    """WebSocket worker for handling a chunk of pairs"""
    global ws_connected
...
async def update_positions():
    ...
    for sym in PAIRS:
        if sym in TK_CACHE:
            price_dict[sym] = TK_CACHE[sym]["last"]
    ...
    while not shutdown_requested:
        try:
            ...
            if results:
                ...
                best = results[:3]  # Top 3 signals
                
                for sig in best:
                    try:
                        # Format message with chart
            ...
            # Send hourly summary
            if (datetime.utcnow() - last_summary).total_seconds() >= 3600:
...
@app.get("/signals")
async def signals():
    """Recent signals with full details"""
