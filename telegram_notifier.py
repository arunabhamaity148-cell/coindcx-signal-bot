import requests
from typing import Dict
from config import config

class TelegramNotifier:
    """
    Sends premium formatted trading signals to Telegram
    """
    
    @staticmethod
    def send_signal(signal: Dict):
        """
        Send trading signal to Telegram channel/group
        
        Args:
            signal: Signal dictionary with all details
        """
        
        direction_emoji = "🟢" if signal['direction'] == "LONG" else "🔴"
        fire_emoji = "🔥" if signal['score'] >= 80 else "⚡"
        
        # Build premium message
        message = f"""
{fire_emoji} **{signal['direction']} SIGNAL** {direction_emoji}

━━━━━━━━━━━━━━━━━━━━
**📊 PAIR:** `{signal['pair']}`
**💯 SCORE:** {signal['score']}/100
**📈 LEVERAGE:** {signal['leverage']}x

━━━━━━━━━━━━━━━━━━━━
**🎯 ENTRY:** ₹{signal['entry']:,.2f}
**🛑 STOP LOSS:** ₹{signal['sl']:,.2f}
**✅ TP1:** ₹{signal['tp1']:,.2f}
**✅ TP2:** ₹{signal['tp2']:,.2f}

━━━━━━━━━━━━━━━━━━━━
**📉 RSI:** {signal['rsi']}
**📊 ADX:** {signal['adx']} (Trend Strength)
**🔄 MTF:** {signal['mtf_trend']}
**📦 Volume:** {signal['volume_surge']}x avg

━━━━━━━━━━━━━━━━━━━━
**⚙️ MODE:** {signal['mode']}
**⏰ TIME:** {signal['timestamp']}

━━━━━━━━━━━━━━━━━━━━
_Auto-generated by ARUN Bot 🤖_
        """
        
        TelegramNotifier._send_message(message)
    
    @staticmethod
    def send_alert(title: str, message: str):
        """
        Send general alert/notification
        
        Args:
            title: Alert title
            message: Alert message
        """
        
        text = f"""
⚠️ **{title}** ⚠️

{message}
        """
        
        TelegramNotifier._send_message(text)
    
    @staticmethod
    def send_startup_message():
        """Send bot startup notification"""
        
        message = f"""
🚀 **ARUN BOT STARTED** 🚀

━━━━━━━━━━━━━━━━━━━━
**⚙️ Configuration:**
• Mode: {config.MODE}
• Auto Trade: {'✅ ON' if config.AUTO_TRADE else '❌ OFF'}
• Max Signals: {config.MAX_SIGNALS_PER_DAY}/day
• Pairs: {len(config.PAIRS)}

━━━━━━━━━━━━━━━━━━━━
**📊 Trading Pairs:**
{', '.join(config.PAIRS)}

━━━━━━━━━━━━━━━━━━━━
Bot is now monitoring markets 👀
        """
        
        TelegramNotifier._send_message(message)
    
    @staticmethod
    def send_daily_summary(stats: Dict):
        """
        Send end-of-day summary
        
        Args:
            stats: Statistics dictionary
        """
        
        message = f"""
📊 **DAILY SUMMARY** 📊

━━━━━━━━━━━━━━━━━━━━
**Signals Generated:** {stats['signals_today']}
**Mode:** {stats['mode']}
**Success Rate:** {stats.get('success_rate', 'N/A')}

━━━━━━━━━━━━━━━━━━━━
See you tomorrow! 🌙
        """
        
        TelegramNotifier._send_message(message)
    
    @staticmethod
    def _send_message(text: str):
        """
        Internal method to send message via Telegram Bot API
        
        Args:
            text: Message text (supports Markdown)
        """
        
        if not config.TELEGRAM_BOT_TOKEN or not config.TELEGRAM_CHAT_ID:
            print("⚠️ Telegram credentials not configured")
            print(f"Message: {text}")
            return
        
        url = f"https://api.telegram.org/bot{config.TELEGRAM_BOT_TOKEN}/sendMessage"
        
        data = {
            'chat_id': config.TELEGRAM_CHAT_ID,
            'text': text,
            'parse_mode': 'Markdown'
        }
        
        try:
            response = requests.post(url, data=data, timeout=10)
            response.raise_for_status()
            print("✅ Telegram message sent")
            
        except requests.exceptions.RequestException as e:
            print(f"❌ Telegram send failed: {e}")
    
    @staticmethod
    def test_connection():
        """Test Telegram bot connection"""
        try:
            url = f"https://api.telegram.org/bot{config.TELEGRAM_BOT_TOKEN}/getMe"
            response = requests.get(url, timeout=5)
            response.raise_for_status()
            
            bot_info = response.json()
            if bot_info['ok']:
                print(f"✅ Telegram bot connected: @{bot_info['result']['username']}")
                return True
            else:
                print("❌ Telegram bot connection failed")
                return False
                
        except requests.exceptions.RequestException as e:
            print(f"❌ Telegram test failed: {e}")
            return False